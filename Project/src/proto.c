...
extern "C" {
	...
	/*************************       全局宏定义       *************************/
#define CHECK_DEV_PWD(x)                      (0 == strncmp((char const *)x, g_ccfg_config.m_dev_pwd), MAX_DEV_PASSWD_SIZE)
#define CHECK_FLGS(x,y)                       (x == s_common_pack->m_flg_2 && y == s_common_pack->m_flg_3)

#define ASSERT_IP(x)                          (x <= 0 || x >= 0xff)
#define ASSERT_PORT(x)                        (x <= 0 || x >= 0xffff)
...
#define NB_INTO_PSM_STR                       "ENTER PSM"
	...
	/* 对心跳包的反馈 */
	const char b_udp_respond[] = { 0xEF,0xA3,0xB5,0x3F,0x31,0x30,0x03,0x00 };    /*{ 0xff,0xDD,0x02,0x31,0x30,0x03, 0 };*/
		...
		*p_index = 0;
		...
		else if (PKG_HEARTBEAT == pkg_type) 
		{
			...
			if (TRUE == g_run_paramter.m_gps_ok) 
			{
				...
				{
					(*p_index) += sprintf((char*)p_buff + (*p_index), "%s", "0H*");
				}
			}
			else 
			{
				...
				{
					(*p_index) += sprintf((char*)p_buff + (*p_index), "%s", "1H%");
				}
			}
		}
		return (*p_index);
	...
	/**
	  ...
	  * ***********************************************************************
	  */
	uint8_t p_proto_create_heartbeat(void)
	{
		uint16_t w_ptr = 0;
		...
		/* 构建数据包的头部 */
		w_ptr = p_proto_create_heartbeat(g_buff.m_send_buff, &g_buff.m_send_index, PKG_HEARTBEAT);
		...
		if (g_run_paramter.m_gps_ok == TRUE) 
		{
			...
			w_ptr += sprintf((char*)g_buff.m_send_buff + w_ptr, "%s,%c,%s,%c,%s,%s*%s,%s,%03u**",
				g_gps_info.m_long, g_gps_info.m_long_EW,    // 经度与标号
				g_gps_info.m_lat, g_gps_info.m_lat_NS,      // 纬度与标号
				g_gps_info.m_speed, g_gps_info.m_direct,    // 速度与方向
				g_gps_info.m_data, g_gps_info.m_time,       // 日期与时间
				++g_run_paramter.m_pkg_num % 1000);         // 发送的数据包编号
		}
		else
		{
			/* 基站定位 */
#if 0
			w_ptr += dec2dms((char*)g_buff.m_send_buff + w_ptr, (char*)g_gps_info.m_long);       // 经度
			...
			w_ptr += sprintf((char*)g_buff.m_send_buff + w_ptr, ",N,000.0,000.0%%000000,000000.%03u%%%%",
				++g_run_paramter.m_pkg_num % 1000);                                              //纬度标号及发送的数据包编号
#endif
	...
	/**
	  ...
	  * ***********************************************************************
	  */
	void p_proto_creat_iot_queryid(void)
	{
		uint16_t w_ptr = 1;
	...
	void p_proto_gen_iotid(void)
	{
		...
		{
			uint8_t conv = g_run_paramter.m_imei[i + 2];
	...
	void p_proto_reply_setting_1(void)
	{
		...
		p_nb_port_send_data(g_buff.m_send_buff, w_ptr);
	}
