...
extern "C" {
	...
	void p_init_timer(void)
	{
		...
		*/
		...
		TIM2_ClearFlag(TIM2_FLAG_Update);    // 清除溢出中断标志位
		...
		*/
		...
		TIM4_ClearFlag(TIM4_FLAG_Update);    // 清除溢出中断标志位
	...
	void p_init_BC20_uart(uint32_t BaudRate)
	{
		...
		GPIO_Init(BC20_RXD_PORT, BC20_RXD_PIN, GPIO_Mode_In_FL_No_IT);    // 浮空输入
	...
	void p_init_judge_reset(void)
	{
		...
		GPIO_Init(BC20_RXD_PORT, BC20_RXD_PIN, GPIO_Mode_Out_PP_High_Fast);    // 接收设置为高
		GPIO_Init(BC20_TXD_PORT, BC20_TXD_PIN, GPIO_Mode_In_FL_No_IT);    // 发送设置为浮空
		...
		if (GPIO_ReadInputDataBit(BC20_TXD_PORT,BC20_TXD_PIN))
		{
			...
			GPIO_ResetBits(LED_REGISTER_PORT, LED_REGISTER_PIN);    //拉低NB联网灯
			...
			GPIO_SetBits(LED_SHAKE_PORT, LED_SHAKE_PIN);    //拉高震动灯
			GPIO_SetBits(LED_REGISTER_PORT, LED_REGISTER_PIN);    //拉高NB联网灯
