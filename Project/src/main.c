...
#define DEBUG_RXD_PIN                         SWITCH_THREE_PIN
	...
	void main(void)
	{
		...
		GPIO_Init(BC20_TXD_PORT, BC20_TXD_PIN, GPIO_Mode_Out_PP_High_Fast);
		p_init_BC20_GPIO();
		...
		while (1)
		{
			...
			GPIO_WriteBit(DEBUG_TXD_PORT, DEBUG_TXD_PIN, GPIO_ReadInputDataBit(BC20_RXD_PORT, BC20_RXD_PIN));

			/* 读取 MCU 发来的数据，给ME3616_TX */
			GPIO_WriteBit(BC20_TXD_PORT, BC20_TXD_PIN, GPIO_ReadInputDataBit(DEBUG_RXD_PORT, DEBUG_RXD_PIN));
		}
#else
		...
		p_init_gpio();                      // 初始化管脚
		p_init_IWDG();
		p_init_judge_reset();               // 判断设备是否需要格式化
		...
		while (1)
		{
			...
			{
				if ((g_run_paramter.m_flg_nb_net = p_nb_establish_network()) == FALSE) 
				{
				...
				else
				{
					...
					if (FALSE == g_run_paramter.m_flg_en_unreg
						...
						&& TRUE == g_ccfg_config.m_en_tim_rep)     // 使能了定时上报
					{
			...
			if (s_main_1s_cd != GET_1S_TIMER() && (s_main_1s_cd = GET_1S_TIMER())) 
			{
				...
				if (TRUE == g_run_paramter.m_flg_alarm) 
				{
					...
					{
						/* 查询定位 */
						if (!p_nb_common_func(TRUE, "QGNSSRD=\"NMEA/RMC\"", NULL, 300, NULL, TRUE))    //查询定位信息失败
						{
				...
				/* 检查服务器下发数据 */
				p_nb_common_func(FALSE, NULL, NULL, 10, NULL, FALSE);
	...
	static void register_process(void)
	{
		...
		if (GET_1S_TIMER() != s_1s_cd && (s_1s_cd = GET_1S_TIMER())) 
		{
			...
			/* 发送心跳包 */
			if (0 >= --g_run_paramter.m_tim_heardbeat) 
			{
				...
				if (TRUE == g_run_paramter.m_flg_alarm) 
				{
					g_run_paramter.m_tim_heardbeat = g_ccfg_config.m_tim_alm_interval;
				}
				else
				{
	...
	static uint8_t tim_5s_nc_check_net(void)
	{
		/* 检查NB的网络状态 */
#if (NB_COAP_EN == 1)
		if (TRUE==p_nb_get_cereg())
#else
		if (TRUE==p_nb_check_net())
#endif
		{
