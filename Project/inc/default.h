...
#define _VERSION(data,type)              "version5.0" data " " type
...
#define UDP_MAX_RESET_CNT               3                          // UDP数据包重发次数
#endif // USE_UDP
...
#define DEFAULT_IWDG_TIME               12000
	...
	/* 设备 默认APN */
#define DEFAULT_DEV_APN                 ctnb                       //电信NB-IOT业务中ctnb是省电模式的APN，开启PSM功能
...
#define DEFAULT_TIME_REP                TRUE
	...
	/* 使能关机延时 */
#define DEFAULT_PWR_0FF_DELAY           TRUE
...
#define MAX_TIM_PWR_DELAY               60
	...
	/* 定时报告周期 小时 */
#define DEFAULT_TIM_SLEEP               720
#define MIN_TIM_SLEEP                   1    
#define MAX_TIM_SLEEP                   999

	/* 上报GPS的时间 分 */
#define DEFAULT_TIM_GPS_REP             15
...
#define MAX_TIM_ALM_CONTINUE            9
	...
	/* 报警后GPS的上报间隔（秒） */
#define DEFAULT_TIM_ALM_INTERAL         15
#define MIN_TIM_ALM_INTERAL             5
#define MAX_TIM_ALM_INTERAL             60
...
#define CONTINUE_LOCATE_TIME            10  
...
#define DEV_GUID_ASSERT()               while (STRLEN(DEFAULT_DEV_GUID) > sizeof(g_ccfg_config.m_dev_guid))
	...
	/* 默认身份ID 异常检查 */
#define DEV_IDEN_ID_ASSRET()            while (STRLEN(DEFAULT_DEV_IDEN_ID) > sizeof(g_ccfg_config.m_dev_iden_id))
