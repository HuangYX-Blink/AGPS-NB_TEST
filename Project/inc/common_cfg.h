...
typedef struct 
{
	...
	/* 设备各项使能标志 8 byte */
	uint8_t     m_en_lto5_rep;                                         // 使能1-5组电话上报
...
typedef struct 
{
	...
	uint16_t    m_tim_gps_rep;										    // GPS数据上报时间 心跳
	uint16_t    m_tim_heardbeat;									    // 心跳的时间
	uint16_t    m_tim_alm_continue;								        // 报警的持续时间
	...
	uint16_t    m_switch_trigger_cnt;                                   // 水银开关触发次数

	uint16_t    m_pkg_num;                                              // 发送的数据包数量
